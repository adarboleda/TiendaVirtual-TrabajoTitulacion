-- ============================================
-- SCRIPT: Seeds para AUTH-SERVICE con Emprendedores
-- Descripción: Usuarios, roles y emprendedores de prueba
-- ============================================

BEGIN;

-- ============================================
-- PASO 1: Insertar Usuarios
-- ============================================

-- Admin del sistema
INSERT INTO public.usuarios (username, email, password, nombre, apellido, telefono, activo)
VALUES 
('admin', 'admin@feriadigital.com', '$2a$10$7qQ1q8kNQf7tIYtG1s0qUu2gG6Zc3U5m9KpJx3HqX1zQ8o9sF1B3u', 'Administrador', 'Sistema', '0999999999', true)
ON CONFLICT (username) DO NOTHING;

-- Emprendedores
INSERT INTO public.usuarios (username, email, password, nombre, apellido, telefono, activo)
VALUES 
-- Password: ultimoinca123
('ultimo_inca', 'contacto@ultimoinca.com', '$2a$10$LlOExy4xrjeQrf97migX6ekemzoyHMzseC1bsvt4RCKzedAvvTQrq', 'María', 'Quispe', '0997279323', true),

-- Password: sigcholac123
('sigcholac', 'sigcholac@gmail.com', '$2a$10$kywJj9SI7ZW2CWuNlBPY/uFmZjjM9TuDakkeD8srL9TZtma59zsAS', 'Pedro', 'Guamán', '0992000198', true),

-- Password: perlaandina123
('perla_andina', 'perlaandina@gmail.com', '$2a$10$N7qX3vK4mRdP8fLtY2nGzOzY1wH5sK9jP2mQ4rT6vZ8xL3yU5bN2a', 'Ana', 'Morocho', '0993396358', true),

-- Password: grandesfoods123
('grandes_foods', 'info@grandesfoods.com', '$2a$10$P8rY4wL5nSeQ9gMuZ3oHAOAZ2xI6tL0kQ3nR5sU7wA9yM4zV6cO3b', 'Carlos', 'Ramírez', '0993665065', true)
ON CONFLICT (username) DO NOTHING;

-- Cliente de prueba
INSERT INTO public.usuarios (username, email, password, nombre, apellido, telefono, activo)
VALUES 
-- Password: cliente123
('cliente1', 'cliente@ejemplo.com', '$2a$10$R9sZ5xM6oTfR0hNvA4pIBPBA3yJ7uM1lR4oS6tV8xB0zN5Aw7dP4c', 'Juan', 'Pérez', '0993336635', true)
ON CONFLICT (username) DO NOTHING;

-- ============================================
-- PASO 2: Asignar Roles
-- ============================================

-- Admin
INSERT INTO public.usuario_roles (usuario_id, rol)
SELECT u.id, 'ROLE_ADMIN' FROM public.usuarios u WHERE u.username='admin'
ON CONFLICT DO NOTHING;

-- Emprendedores
INSERT INTO public.usuario_roles (usuario_id, rol)
SELECT u.id, 'ROLE_EMP' FROM public.usuarios u WHERE u.username IN ('ultimo_inca', 'sigcholac', 'perla_andina', 'grandes_foods')
ON CONFLICT DO NOTHING;

-- Cliente
INSERT INTO public.usuario_roles (usuario_id, rol)
SELECT u.id, 'ROLE_USER' FROM public.usuarios u WHERE u.username='cliente1'
ON CONFLICT DO NOTHING;

-- ============================================
-- PASO 3: Crear Emprendedores
-- Nota: empresa_id se vinculará después de crear las empresas en productos DB
-- ============================================

-- Emprendedor 1: El Último Inca (empresa_id: 1)
INSERT INTO public.emprendedores (usuario_id, empresa_id, descripcion, activo)
SELECT 
    u.id, 
    1,  -- Se actualizará después de crear empresas
    'Producción artesanal de vinos de frutas andinas. Especialidad en vino de mortiño, frambuesa, pitahaya y maracuyá.',
    true
FROM public.usuarios u 
WHERE u.username = 'ultimo_inca'
ON CONFLICT (usuario_id) DO NOTHING;

-- Emprendedor 2: Sigcholac (empresa_id: 2)
INSERT INTO public.emprendedores (usuario_id, empresa_id, descripcion, activo)
SELECT 
    u.id, 
    2,
    'Producción y comercialización de productos lácteos artesanales: queso fresco, queso mozzarella, yogurt.',
    true
FROM public.usuarios u 
WHERE u.username = 'sigcholac'
ON CONFLICT (usuario_id) DO NOTHING;

-- Emprendedor 3: Perla Andina (empresa_id: 3)
INSERT INTO public.emprendedores (usuario_id, empresa_id, descripcion, activo)
SELECT 
    u.id, 
    3,
    'Vinos premium de frutas andinas. Especialidad en vinos dulces y secos.',
    true
FROM public.usuarios u 
WHERE u.username = 'perla_andina'
ON CONFLICT (usuario_id) DO NOTHING;

-- Emprendedor 4: Grandes Foods (empresa_id: 4)
INSERT INTO public.emprendedores (usuario_id, empresa_id, descripcion, activo)
SELECT 
    u.id, 
    4,
    'Producción de lácteos y productos cárnicos. Chochos, salchichas, longanizas y morcillas.',
    true
FROM public.usuarios u 
WHERE u.username = 'grandes_foods'
ON CONFLICT (usuario_id) DO NOTHING;

COMMIT;

-- ============================================
-- VERIFICACIÓN
-- ============================================
SELECT 'Usuarios creados:' AS info;
SELECT id, username, email, nombre, apellido FROM public.usuarios;

SELECT 'Roles asignados:' AS info;
SELECT u.username, ur.rol 
FROM public.usuario_roles ur
JOIN public.usuarios u ON ur.usuario_id = u.id;

SELECT 'Emprendedores creados:' AS info;
SELECT e.id, u.username, u.nombre, u.apellido, e.empresa_id, e.descripcion
FROM public.emprendedores e
JOIN public.usuarios u ON e.usuario_id = u.id;
